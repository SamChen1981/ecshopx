    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $data_format_array = array(
                ...
                'paipai3' => $GLOBALS['_LANG']['export_paipai3'],
                'taobao46' => $GLOBALS['_LANG']['export_taobao46'],
            );
            $GLOBALS['smarty']->assign('data_format', $data_format_array);
            $GLOBALS['smarty']->assign('lang_list', $lang_list);
            $GLOBALS['smarty']->assign('download_list', $download_list);
            ...
            /* 显示模板 */
            assign_query_info();
            return $GLOBALS['smarty']->display('goods_batch_add.view.php');
        }
        ...
        elseif ($_REQUEST['act'] == 'upload') {
            ...
            if ($_POST['data_cat'] == 'ecshop') {
                ...
                foreach ($data as $line) {
                    ...
                    $max_id = $GLOBALS['db']->getOne("SELECT MAX(goods_id) + $id_is FROM " . $GLOBALS['ecs']->table('goods'));
                    $id_is++;
            ...
            $GLOBALS['smarty']->assign('goods_class', $GLOBALS['_LANG']['g_class']);
            $GLOBALS['smarty']->assign('goods_list', $goods_list);

            // 字段名称列表
            $GLOBALS['smarty']->assign('title_list', $GLOBALS['_LANG']['upload_goods']);

            // 显示的字段列表
            $GLOBALS['smarty']->assign('field_show', array('goods_name' => true, 'goods_sn' => true, 'brand_name' => true, 'market_price' => true, 'shop_price' => true));

            /* 参数赋值 */
            $GLOBALS['smarty']->assign('ur_here', $GLOBALS['_LANG']['goods_upload_confirm']);

            /* 显示模板 */
            assign_query_info();
            return $GLOBALS['smarty']->display('goods_batch_confirm.view.php');
        }
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if (isset($_POST['checked'])) {
                ...
                foreach ($_POST['checked'] as $key => $value) {
                        ...
                        if (!empty($field_value)) {
                            ...
                            elseif ($field == 'brand_name') {
                                if (isset($brand_list[$field_value])) {
                                    ...
                                    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('brand') . " (brand_name) VALUES ('" . addslashes($field_value) . "')";
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods'), $field_arr, 'INSERT');

                    $max_id = $GLOBALS['db']->insert_id() + 1;
                    ...
                    if (!empty($field_arr['original_img']) || !empty($field_arr['goods_img']) || !empty($field_arr['goods_thumb'])) {
                        ...
                        $original_img = '';
                        $goods_gallery = array();
                        $goods_gallery['goods_id'] = $GLOBALS['db']->insert_id();
                        ...
                        //修改商品图
                        $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('goods') . " SET goods_img = '$goods_img', goods_thumb = '$goods_thumb', original_img = '$original_img' WHERE goods_id='" . $goods_gallery['goods_id'] . "'");
        ...
        elseif ($_REQUEST['act'] == 'select') {
            ...
            /* 显示模板 */
            assign_query_info();
            return $GLOBALS['smarty']->display('goods_batch_select.view.php');
        }
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            if ($_POST['select_method'] == 'cat') {
                ...
                $goods_ids = join(',', $GLOBALS['db']->getCol($sql));
                $where = " WHERE goods_id " . db_create_in($goods_ids);
            }
            $sql = "SELECT DISTINCT goods_id, goods_sn, goods_name, market_price, shop_price, goods_number, integral, give_integral, brand_id, is_real FROM " . $GLOBALS['ecs']->table('goods') . $where;
            $GLOBALS['smarty']->assign('goods_list', $GLOBALS['db']->getAll($sql));

            /* 取编辑商品的货品列表 */
            $product_exists = false;
            $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('products') . $where;
            $product_list = $GLOBALS['db']->getAll($sql);
            ...
            if (!empty($product_list)) {
                ...
                $GLOBALS['smarty']->assign('product_list', $_product_list);

                //释放资源
                unset($product_list, $sql, $_product_list);
            }

            $GLOBALS['smarty']->assign('product_exists', $product_exists);
            ...
            $GLOBALS['smarty']->assign('rank_list', $GLOBALS['db']->getAll($sql));

            /* 取得品牌列表 */
            $GLOBALS['smarty']->assign('brand_list', get_brand_list());

            /* 赋值编辑方式 */
            $GLOBALS['smarty']->assign('edit_method', $_POST['edit_method']);
            ...
            /* 显示模板 */
            assign_query_info();
            return $GLOBALS['smarty']->display('goods_batch_edit.view.php');
        }
                    ...
                    foreach ($_POST['goods_id'] as $goods_id) {
                        ...
                        if (!empty($_POST['product_number'][$goods_id])) {
                            ...
                            foreach ($_POST['product_number'][$goods_id] as $key => $value) {
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('products'), array('product_number', $value), 'UPDATE', "goods_id = '$goods_id' AND product_id = " . $key);
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods'), $goods, 'UPDATE', "goods_id = '$goods_id'");
                        ...
                        if (!empty($_POST['rank_id'])) {
                            foreach ($_POST['rank_id'] as $rank_id) {
                                ...
                                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'";
                                if ($GLOBALS['db']->getOne($sql) > 0) {
                                    if ($rank['user_price'] < 0) {
                                        $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['ecs']->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'");
                                    } else {
                                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('member_price'), $rank, 'UPDATE', "goods_id = '$goods_id' AND user_rank = '$rank_id'");
                                    }
                                } else {
                                    if ($rank['user_price'] >= 0) {
                                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('member_price'), $rank, 'INSERT');
                                    }
                                }
                            }
                        }
