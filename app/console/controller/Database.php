...
class Database extends Init
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'backup') {
            ...
            return $GLOBALS['smarty']->display('db_backup.view.php');
        }
        ...
        if ($_REQUEST['act'] == 'restore') {
            ...
            return $GLOBALS['smarty']->display('db_restore.view.php');
        }
            ...
            if (empty($tables)) {
                ...
                if ($vol > 1) {
                    ...
                    if (!@file_put_contents(ROOT_PATH . DATA_DIR . '/sqldata/' . $sql_file_name . '_' . $vol . '.sql', $dump->dump_sql)) {
                        return sys_msg(sprintf($GLOBALS['_LANG']['fail_write_file'], $sql_file_name . '_' . $vol . '.sql'), 1, array(array('text' => $GLOBALS['_LANG']['02_db_manage'], 'href' => 'database.php?act=backup')), false);
                    }
                    $list = array();
                    for ($i = 1; $i <= $vol; $i++) {
                    ...
                    return $GLOBALS['smarty']->display('sql_dump_msg.view.php');
                } else {
                    /* 只有一个文件 */
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            return sys_msg($GLOBALS['_LANG']['remove_success'], 0, array(array('text' => $GLOBALS['_LANG']['restore'], 'href' => 'database.php?act=restore')));
        }
        ...
        if ($_REQUEST['act'] == 'import') {
            ...
            if (preg_match('/_[0-9]+\.sql$/', $file_name)) {
                ...
                if ($is_confirm == false) {
                    /* 提示用户要求确认 */
                    return sys_msg($GLOBALS['_LANG']['confirm_import'], 1, array(array('text' => $GLOBALS['_LANG']['also_continue'], 'href' => 'database.php?act=import&confirm=1&file_name=' . $file_name)), false);
                }
                ...
                return sys_msg($GLOBALS['_LANG']['restore_success'], 0, array(array('text' => $GLOBALS['_LANG']['restore'], 'href' => 'database.php?act=restore')));
            } else {
                /* 单卷 */
        ...
        if ($_REQUEST['act'] == 'upload_sql') {
            ...
            if ($this->sql_import($sql_file)) {
                clear_all_files();
                @unlink($sql_file);
                return sys_msg($GLOBALS['_LANG']['restore_success'], 0, array());
            } else {
                @unlink($sql_file);
                return sys_msg($GLOBALS['_LANG']['sqlfile_error'], 1);
            }
        ...
        if ($_REQUEST['act'] == 'optimize') {
            ...
            return $GLOBALS['smarty']->display('optimize.view.php');
        }
        ...
        if ($_REQUEST['act'] == 'run_optimize') {
            ...
            return sys_msg(sprintf($GLOBALS['_LANG']['optimize_ok'], $_POST['num']), 0, array(array('text' => $GLOBALS['_LANG']['go_back'], 'href' => 'database.php?act=optimize')));
        }
        ...
        if ($_REQUEST['act'] == 'clear') {
            ...
            return $GLOBALS['smarty']->display('clear.view.php');
        }
        ...
        if ($_REQUEST['act'] == 'cleardata') {
            ...
            if ((!$data['username'] or !$data['password']) && !$_SESSION['yunqi_login']) {
                return sys_msg($GLOBALS['_LANG']['manage_required']);
            }
            $msg = '';
            if ($_SESSION['yunqi_login'] or tryLogin($data, $msg)) {
                ...
                return sys_msg($GLOBALS['_LANG']['clear_success']);
            } else {
                return sys_msg($msg);
            }
        }
