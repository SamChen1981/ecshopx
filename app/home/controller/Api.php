...
function search_goods_detail()
{
    ...
    if (!empty($_POST['goods_id']) && is_numeric($_POST['goods_id'])) {
        ...
        $goods_data['image_default'] = defined('FORCE_SSL_LOGIN') ? 'https://' : 'http://' . $_SERVER['HTTP_HOST'] . '/' . $goods_data['image_default'];
        $goods_data['unit'] = '千克';
...
function attribute($goods_data, $goods, $type_id)
{
    ...
    $filter = array('颜色分类', '尺码', '鞋码', '套餐类型'); //淘宝定义属性，过滤(特殊处理)
    foreach ($attr_arr as $k => $v) {
        if (trim($v) == '' || in_array($k, $filter)) {
            unset($attr_arr[$k]);
        } else {
            $attr_arr[$k] = array($v);
        }
    ...
    if ($res = $db->getAll("SELECT * FROM " . $ecs->table('goods_attr') . " AS g LEFT JOIN " . $ecs->table('attribute') . " AS a ON g.attr_id = a.attr_id WHERE g.goods_id = {$goods['goods_id']}")) {
        ...
        foreach ($res as $v) {
            if (!empty($v['attr_values'])) {
                $tmp_attr_values_arr = array();
                $tmp_attr_values_arr = explode("\n", $v['attr_values']);
                // 去除属性值后面的空白字符(此次主要针对回车符)  不然下面in_array判断是否已在数组里时有时会出现问题 2019.01.08 18:10:00 pangxp
                foreach ($tmp_attr_values_arr as &$tVal) {
                    $tVal = rtrim($tVal);
                }
                $res_arr[$v['attr_name']][$v['attr_id']] = $tmp_attr_values_arr;
        ...
        foreach ($res_arr as $name => $values) {
            ...
            if ($goods_attr_arr[$name]) {
                ...
                if (empty($goods_attr_arr[$name])) {
                    unset($goods_attr_arr[$name]);
                }
            }
        }
    }
...
function shopex_goods_search()
{
    ...
    if ($res = $db->getAll($sql)) {
        foreach ($res as $k => $goods) {
            ...
            $prop_values = $products = $spec_values = array();
...
//商品属性去掉引号处理
function remove_quotation_marks($data)
{
