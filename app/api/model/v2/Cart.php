...
class Cart extends BaseModel
{
    ...
    public static function add(array $attributes)
    {
        ...
        return self::formatBody(['cart_goods' => $parent]);
    }
    ...
    public static function remove(array $attributes)
    {
        ...
        $decode_good = json_decode($good, true);

        if (is_array($decode_good)) {
            Cart::where('rec_type', self::CART_GENERAL_GOODS)->whereIn('rec_id', $decode_good)->delete();
        } else {
    ...
    {
        return self::formatBody(['quantity' => self::join('goods', 'goods.goods_id', '=', 'cart.goods_id')->where('user_id', Token::authorization())->where('goods.is_on_sale', 1)
            ->where('goods.is_delete', 0)->sum('cart.goods_number')]);
    }
    ...
    {
        return FavourableActivity::getPromoByGoods($this->goods_id, $this->product->category, $this->product->brand, true);
    }
    ...
    public static function compute_discount($order_products)
    {
        ...
        //请求商派接口
        return Logistics::discount($goods_list);
        ...
        // // 循环计算每个优惠活动的折扣
        // // 优惠等级：主级：按金额下限由高到低 ；次级：按部分商品 > 按品牌 > 按分类 > 按订单(全部商品)
        // // 按订单时，在以上优惠完成后的总额基础上再做按订单优惠;
        // $all_amount = 0 ;
        // foreach ($favourable_list as $favourable) {
        //     if ($favourable['act_range'] == FavourableActivity::FAR_ALL) {
        //         foreach ($goods_list as $k=>$goods) {
        //             $all_amount += $goods['price'] * $goods['amount']; // 获取未进行任何优惠时的购物车所有商品总额
        //         }
        //         log::debug('全部商品总额');
        //         break; // 如果设有多个[按订单(全部商品)]优惠活动，只取金额下限最高的第一个，下限相同时视为同一个活动且取优惠折扣额度最高的第一个
        //     }
        // }
        // foreach ($favourable_list as $favourable) {
        //     Log::debug("开始循环");
        //     $total_amount = 0;
        //     $key = '';
        //     if ($favourable['act_range'] == FavourableActivity::FAR_GOODS) {
        //         foreach ($goods_list as $k=>$goods) {
        ...
        //         continue;
        //     }
        //     // 计算购物车中某商品参与某一活动后获得的折扣额度
        //     if ($key !== '' && $total_amount > 0 && $total_amount >= $favourable['min_amount'] && ($total_amount <= $favourable['max_amount'] || $favourable['max_amount'] == 0)) {
        //         if ($favourable['act_type'] == FavourableActivity::FAT_DISCOUNT) {
        //             Log::debug("新增优惠逻辑");
        //             Log::debug("记录递归传递值".$total_amount);
        //             Log::debug("递归数组".json_encode(FavourableActivity::discount($total_amount)));
        //             //$discount = array_sum(FavourableActivity::discount($total_amount));
        //             $discount += $total_amount * (1 - $favourable['act_type_ext'] / 100);
        //             $tmp_goods_ids = explode('-', $key);
        //             for($i = 0; $i < count($tmp_goods_ids); $i++){
        //                 unset($goods_list[$tmp_goods_ids[$i]]); // 去掉已参与优惠活动的商品
        //             }
        ...
        // foreach ($favourable_list as $favourable){
        //     if ($favourable['act_range'] == FavourableActivity::FAR_ALL) {
        //         $all_amount = $all_amount - $discount ; // 获取品牌、分类、部分商品活动优惠后的订单总额
        //         if ($all_amount > 0 && $all_amount >= $favourable['min_amount'] && ($all_amount <= $favourable['max_amount'] || $favourable['max_amount'] == 0)){
        //             if ($favourable['act_type'] == FavourableActivity::FAT_DISCOUNT) {
        //                 $discount += $all_amount * (1 - $favourable['act_type_ext'] / 100);
        //             } elseif ($favourable['act_type'] == FavourableActivity::FAT_PRICE) {
        //                 $discount += $favourable['act_type_ext'];
        //             }
        //             log::debug('获得总折扣'.$discount);
        //         }
        //     }
        //     break; // 取得一个按订单优惠的活动即终止循环,且为金额下限和优惠额度最高的一个
        // }
        // return $discount;
    }
