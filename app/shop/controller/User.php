...
class User extends Init
{
    public function index()
    {
        ...
        if ($action == 'register') {
            ...
            return $this->fetch('user_passport');
        } /* 注册会员的处理 */
        elseif ($action == 'act_register') {
            ...
            if ($GLOBALS['_CFG']['shop_reg_closed']) {
                ...
                if (empty($_POST['agreement'])) {
                    return $this->show_message($GLOBALS['_LANG']['passport_js']['agreement']);
                }
                if (strlen($username) < 3) {
                    return $this->show_message($GLOBALS['_LANG']['passport_js']['username_shorter']);
                }
                if (is_numeric($username) && !preg_match("/^(((13[0-9]{1})|(14[0-9]{1})|(15[0-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/", $username)) {
                    return $this->show_message($GLOBALS['_LANG']['passport_js']['username_invalid']);
                }

                if (strlen($password) < 6) {
                    return $this->show_message($GLOBALS['_LANG']['passport_js']['password_shorter']);
                }

                if (strpos($password, ' ') > 0) {
                    return $this->show_message($GLOBALS['_LANG']['passwd_balnk']);
                }
                ...
                if (preg_match("/^1[34578]{1}\d{9}$/", $username)) {
                    if ($sms_code != '' && $sms_code == $_SESSION[$username . '-sms_code']) {
                        ...
                        if (time() - $_SESSION[$username . '-send_time'] > 600) {
                            return $this->show_message('短信验证码已过期', '返回上一页', 'user.php?act=register', 'error');
                        }
                        $other['mobile_phone'] = $username;
                        $_SESSION[$username . '-sms_code'] = 'false';
                    } else {
                        return $this->show_message('短信验证码错误', $GLOBALS['_LANG']['sign_up'], 'user.php?act=register', 'error');
                    }
                }
                ...
                if (register($username, $password, $email, $other) !== false) {
                    ...
                    return $this->show_message(sprintf($GLOBALS['_LANG']['register_success'], $username . $ucdata), array($GLOBALS['_LANG']['back_up_page'], $GLOBALS['_LANG']['profile_lnk']), array($back_act, 'user.php'), 'info');
                } else {
            ...
            if ($hash) {
                ...
                if ($id > 0) {
                    ...
                    $row = $GLOBALS['db']->getRow($sql);
                    return $this->show_message(sprintf($GLOBALS['_LANG']['validate_ok'], $row['user_name'], $row['email']), $GLOBALS['_LANG']['profile_lnk'], 'user.php');
                }
            }
            return $this->show_message($GLOBALS['_LANG']['validate_fail']);
        } /* 验证用户注册用户名是否可以注册 */
        ...
        elseif ($action == 'act_login') {
            ...
            // 登录密码不能为空
            if (empty($password)) {
                return $this->show_message($GLOBALS['_LANG']['passport_js']['password_empty'], $GLOBALS['_LANG']['relogin_lnk'], 'user.php', 'error');
            }
            ...
            if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0) {
                if (empty($_POST['captcha'])) {
                    return $this->show_message($GLOBALS['_LANG']['invalid_captcha'], $GLOBALS['_LANG']['relogin_lnk'], 'user.php', 'error');
                }
            ...
            if ($GLOBALS['user']->login($username, $password, isset($_POST['remember']))) {
                ...
                return $this->show_message($GLOBALS['_LANG']['login_success'] . $ucdata, array($GLOBALS['_LANG']['back_up_page'], $GLOBALS['_LANG']['profile_lnk']), array($back_act, 'user.php'), 'info');
            } else {
                $_SESSION['login_fail']++;
                return $this->show_message($GLOBALS['_LANG']['login_failure'], $GLOBALS['_LANG']['relogin_lnk'], 'user.php', 'error');
            }
        } /* 处理 ajax 的登录请求 */
        elseif ($action == 'signin') {
            ...
            if ($GLOBALS['user']->login($username, $password)) {
                ...
                $result['content'] = $GLOBALS['smarty']->fetch('library/member_info');
            } else {
                $_SESSION['login_fail']++;
                if ($_SESSION['login_fail'] > 2) {
                    $this->assign('enabled_captcha', 1);
                    $result['html'] = $GLOBALS['smarty']->fetch('library/member_info');
                }
                $result['error'] = 1;
        ...
        elseif ($action == 'logout') {
            ...
            $GLOBALS['user']->logout();
            $ucdata = empty($GLOBALS['user']->ucdata) ? "" : $GLOBALS['user']->ucdata;
            return $this->show_message($GLOBALS['_LANG']['logout'] . $ucdata, array($GLOBALS['_LANG']['back_up_page'], $GLOBALS['_LANG']['back_home_lnk']), array($back_act, 'index.php'), 'info');
        } /* 个人资料页面 */
        elseif ($action == 'profile') {
            ...
            return $this->fetch('user_transaction');
        } /* 修改个人资料的处理 */
        elseif ($action == 'act_edit_profile') {
            ...
            if (!empty($office_phone) && !preg_match('/^[\d|\_|\-|\s]+$/', $office_phone)) {
                return $this->show_message($GLOBALS['_LANG']['passport_js']['office_phone_invalid']);
            }
            if (!empty($home_phone) && !preg_match('/^[\d|\_|\-|\s]+$/', $home_phone)) {
                return $this->show_message($GLOBALS['_LANG']['passport_js']['home_phone_invalid']);
            }
            if (!is_email($email)) {
                return $this->show_message($GLOBALS['_LANG']['msg_email_format']);
            }
            if (!empty($msn) && !is_email($msn)) {
                return $this->show_message($GLOBALS['_LANG']['passport_js']['msn_invalid']);
            }
            if (!empty($qq) && !preg_match('/^\d+$/', $qq)) {
                return $this->show_message($GLOBALS['_LANG']['passport_js']['qq_invalid']);
            }
            if (!empty($mobile_phone) && !preg_match('/^[\d-\s]+$/', $mobile_phone)) {
                return $this->show_message($GLOBALS['_LANG']['passport_js']['mobile_phone_invalid']);
            }
            ...
            if (edit_profile($profile)) {
                return $this->show_message($GLOBALS['_LANG']['edit_profile_success'], $GLOBALS['_LANG']['profile_lnk'], 'user.php?act=profile', 'info');
            } else {
        ...
        elseif ($action == 'get_password') {
            ...
            if (isset($_GET['code']) && isset($_GET['uid'])) { //从邮件处获得的act
                ...
                if (empty($user_info) || ($user_info && md5($user_info['user_id'] . $GLOBALS['_CFG']['hash_code'] . $user_info['reg_time']) != $code)) {
                    return $this->show_message($GLOBALS['_LANG']['parm_error'], $GLOBALS['_LANG']['back_home_lnk'], './', 'info');
                }
        ...
        elseif ($action == 'get_passwd_question') {
            if (empty($_POST['user_name'])) {
                return $this->show_message($GLOBALS['_LANG']['no_passwd_question'], $GLOBALS['_LANG']['back_home_lnk'], './', 'info');
            } else {
        ...
        elseif ($action == 'check_answer') {
            ...
            if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0) {
                if (empty($_POST['captcha'])) {
                    return $this->show_message($GLOBALS['_LANG']['invalid_captcha'], $GLOBALS['_LANG']['back_retry_answer'], 'user.php?act=qpassword_name', 'error');
                }
            ...
            if (empty($_POST['passwd_answer']) || $_POST['passwd_answer'] != $_SESSION['passwd_answer']) {
                return $this->show_message($GLOBALS['_LANG']['wrong_passwd_answer'], $GLOBALS['_LANG']['back_retry_answer'], 'user.php?act=qpassword_name', 'info');
            } else {
        ...
        elseif ($action == 'send_pwd_email') {
            ...
            if ($user_info && $user_info['email'] == $email) {
                ...
                if (send_pwd_email($user_info['user_id'], $user_name, $email, $code)) {
                    return $this->show_message($GLOBALS['_LANG']['send_success'] . $email, $GLOBALS['_LANG']['back_home_lnk'], './', 'info');
                } else {
                    //发送邮件出错
                    return $this->show_message($GLOBALS['_LANG']['fail_send_password'], $GLOBALS['_LANG']['back_page_up'], './', 'info');
                }
            } else {
                //用户名与邮件地址不匹配
                return $this->show_message($GLOBALS['_LANG']['username_no_email'], $GLOBALS['_LANG']['back_page_up'], '', 'info');
            }
        } /* 重置新密码 */
        ...
        elseif ($action == 'act_edit_password') {
            ...
            if (($user_info && (!empty($code) && md5($user_info['user_id'] . $GLOBALS['_CFG']['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id'] > 0 && $_SESSION['user_id'] == $user_id && $GLOBALS['user']->check_user($_SESSION['user_name'], $old_password))) {
                if ($GLOBALS['user']->edit_user(array('username' => (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'old_password' => $old_password, 'password' => $new_password), empty($code) ? 0 : 1)) {
                    $sql = "UPDATE " . $GLOBALS['ecs']->table('users') . "SET `ec_salt`='0' WHERE user_id= '" . $user_id . "'";
                    $GLOBALS['db']->query($sql);
                    $GLOBALS['user']->logout();
                    return $this->show_message($GLOBALS['_LANG']['edit_password_success'], $GLOBALS['_LANG']['relogin_lnk'], 'user.php?act=login', 'info');
                } else {
                    return $this->show_message($GLOBALS['_LANG']['edit_password_failure'] . '1', $GLOBALS['_LANG']['back_page_up'], '', 'info');
                }
            } else {
                return $this->show_message($GLOBALS['_LANG']['edit_password_failure'], $GLOBALS['_LANG']['back_page_up'], '', 'info');
            }
        } /* 添加一个红包 */
        elseif ($action == 'act_add_bonus') {
            ...
            if (add_bonus($user_id, $bouns_sn)) {
                return $this->show_message($GLOBALS['_LANG']['add_bonus_sucess'], $GLOBALS['_LANG']['back_up_page'], 'user.php?act=bonus', 'info');
            } else {
        ...
        elseif ($action == 'order_detail') {
            ...
            if (!empty($payment_info)) {
                ...
                echo json_encode(array('status' => true, 'pay_online' => $pay_online));

            } else {
                echo json_encode(array('status' => false, 'msg' => '支付方式为空'));
        ...
        elseif ($action == 'account_bind') {
            return $this->fetch('account_bind');
        } /* 添加/编辑收货地址的处理 */
        elseif ($action == 'act_edit_address') {
            ...
            if (update_address($address)) {
                return $this->show_message($GLOBALS['_LANG']['edit_address_success'], $GLOBALS['_LANG']['address_list_lnk'], 'user.php?act=address_list');
            }
        } /* 删除收货地址 */
        elseif ($action == 'drop_consignee') {
            ...
            if (drop_consignee($consignee_id)) {
                ...
                return $this->show_message($GLOBALS['_LANG']['del_address_false']);
            }
        } /* 显示收藏商品列表 */
        ...
        elseif ($action == 'act_add_message') {
            ...
            if (add_message($message)) {
                ...
                // }
                return $this->show_message($GLOBALS['_LANG']['add_message_success'], $GLOBALS['_LANG']['message_list_lnk'], 'user.php?act=message_list&order_id=' . $message['order_id'], 'info');
            } else {
        ...
        elseif ($action == 'add_booking') {
            ...
            if ($goods_id == 0) {
                return $this->show_message($GLOBALS['_LANG']['no_goods_id'], $GLOBALS['_LANG']['back_page_up'], '', 'error');
            }
        ...
        elseif ($action == 'act_add_booking') {
            ...
            if ($rec_id > 0) {
                return $this->show_message($GLOBALS['_LANG']['booking_rec_exist'], $GLOBALS['_LANG']['back_page_up'], '', 'error');
            }
        ...
        elseif ($action == 'act_account') {
            ...
            if ($amount <= 0) {
                return $this->show_message($GLOBALS['_LANG']['amount_gt_zero']);
            }
            ...
            if ($surplus['process_type'] == 1) {
                ...
                if ($amount > $sur_amount) {
                    ...
                    return $this->show_message($content, $GLOBALS['_LANG']['back_page_up'], '', 'info');
                }
                ...
                if ($surplus['rec_id'] > 0) {
                    ...
                    return $this->show_message($content, $GLOBALS['_LANG']['back_account_log'], 'user.php?act=account_log', 'info');
                } else {
                ...
                if ($surplus['payment_id'] <= 0) {
                    return $this->show_message($GLOBALS['_LANG']['select_payment_pls']);
                }
                ...
                if ($surplus['rec_id'] > 0) {
                    ...
                    if (!check_account_money($surplus['rec_id'], $user_id, $amount)) {
                        return $this->show_message($GLOBALS['_LANG']['check_account_money_fail']);
                    }
                    //更新会员账目明细
        ...
        elseif ($action == 'merge_order') {
            ...
            if (merge_user_order($from_order, $to_order, $user_id)) {
                return $this->show_message($GLOBALS['_LANG']['merge_order_success'], $GLOBALS['_LANG']['order_list_lnk'], 'user.php?act=order_list', 'info');
            } else {
        ...
        elseif ($action == 'bonus') {
            ...
            return $this->fetch('user_transaction . dwt');
        } /* 我的团购列表 */
        ...
        elseif ($action == 'email_list') {
            ...
            if ($job == 'add') {
                ...
                return $this->show_message($info, $GLOBALS['_LANG']['back_home_lnk'], 'index.php');
        ...
        ($action == 'order_query') {
            ...
            $result['content'] = $GLOBALS['smarty']->fetch('library/order_query');
            die(json_encode($result));
        } elseif
        ...
        ($action == 'act_transform_points') {
            ...
            if ($num <= 0 || $num != floor($num)) {
                return $this->show_message($GLOBALS['_LANG']['invalid_points'], $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
            }
            ...
            if ($max_points <= 0 || $num > $max_points) {
                return $this->show_message($GLOBALS['_LANG']['overflow_points'], $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
            }
            ...
            switch ($rule_key) {
                case TO_P:
                    ...
                    return $this->show_message(sprintf($GLOBALS['_LANG']['to_pay_points'], $num, $points_name[$bbs_key]['title'], $result_points), $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
                ...
                case TO_R:
                    ...
                    return $this->show_message(sprintf($GLOBALS['_LANG']['to_rank_points'], $num, $points_name[$bbs_key]['title'], $result_points), $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
                ...
                case FROM_P:
                    ...
                    return $this->show_message(sprintf($GLOBALS['_LANG']['from_pay_points'], $num, $result_points, $points_name[$bbs_key]['title']), $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
                ...
                case FROM_R:
                    ...
                    $GLOBALS['user']->set_points($row['user_name'], array($bbs_key => $result_points)); //调整论坛积分
                    return $this->show_message(sprintf($GLOBALS['_LANG']['from_rank_points'], $num, $result_points, $points_name[$bbs_key]['title']), $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
            }
        } elseif
        ($action == 'act_transform_ucenter_points') {
            ...
            if ($ratio == 0) {
                return $this->show_message($GLOBALS['_LANG']['exchange_deny'], $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
            }
            ...
            if ($result === true) {
                ...
                $GLOBALS['db']->query($sql);
                return $this->show_message(sprintf($GLOBALS['_LANG']['exchange_success'], $exchange_amount, $GLOBALS['_LANG']['exchange_points'][$fromcredits], $netamount, $credit['title']), $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
            } else {
                return $this->show_message($GLOBALS['_LANG']['exchange_error_1'], $GLOBALS['_LANG']['transform_points'], 'user.php?act=transform_points');
            }
        } /* 清除商品浏览历史 */
        elseif
        ...
        ($action == 'delivery_info') {
            ...
            if (empty($order_sn)) {
                return $this->show_message($GLOBALS['_LANG']['invalid_order_sn'], '', 'user.php?act=order_list');
            }
        ...
        ($action == 'ajax_delivery_info') {
            ...
            $result['content'] = $GLOBALS['smarty']->fetch('library/delivery_info');
        ...
        ($action == 'sms_get_password') {
            ...
            if ($_POST['sms_code'] != '' && $_POST['username'] != '') {
                ...
                if ($sms_code == $_SESSION[$username . '-sms_code']) {
                    // 短信验证码十分钟内有效
                    if (time() - $_SESSION[$username . '-send_time'] > 600) {
                        return $this->show_message('短信验证码已过期', '返回上一页', 'user.php?act=sms_get_password', 'info');
                    }
                    $user_info = $GLOBALS['user']->get_profile_by_name($username);
                    // 判断会员是否存在
                    if (!$user_info['user_id']) {
                        return $this->show_message(sprintf($GLOBALS['_LANG']['username_inexistent'], $username), $GLOBALS['_LANG']['back_page_up'], 'user.php?act=sms_get_password', 'info');
                    }
                    ...
                    return $this->show_message('短信验证码错误', '返回上一页', 'user.php?act=sms_get_password', 'info');
                }
            } else {
                //显示用户名和email表单
                return $this->fetch('user_passport');
