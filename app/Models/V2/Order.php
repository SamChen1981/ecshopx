...
class Order extends BaseModel
{
    ...
    protected $visible = ['id', 'sn', 'total', 'tax', 'goods', 'payment', 'shipping', 'invoice', 'coupon', 'score', 'use_score', 'cashgift', 'consignee', 'status', 'created_at', 'updated_at', 'canceled_at', 'paied_at', 'shipping_at', 'finish_at', 'discount_price', 'promos', 'extension_code'];
    ...
    public static function findUnpayedId($id)
    {
        return self::where(['order_id' => $id, 'pay_status' => Order::PS_UNPAYED])->first();
    }
    ...
    public static function price(array $attributes)
    {
        ...
        if (isset($is_exchange)) {
            ...
            $product_first = current($product);
            $promo_price = (ExchangeGoods::where('goods_id', $product_first['goods_id'])->value('exchange_integral')) * (count($product));
            $user_score = Member::where('user_id', $uid)->value('pay_points');

            if ($user_score < $promo_price) {
                return self::formatError(self::BAD_REQUEST, trans('message.exchange.exchange_score_not_enough'));
            };

            if (!isset($consignee)) {
                $order_price['shipping_price'] = Goods::price_format(0, false);
            } else {
                $order_price['shipping_price'] = Shipping::shipFee($consignee, $product, $shipping);
            }

            $order_price['total_price'] = $order_price['shipping_price'];
            $order_price['product_price'] = 0.00;
            $order_price['discount_price'] = 0.00;
            $order_price['promos'][] = [
                'promo' => 'score',
                'price' => $promo_price
            ];
        ...
        $order_price['tax_price'] = Goods::price_format($total['tax'], false);
    ...
    public static function cancel(array $attributes)
    {
        ...
        if ($model = self::where(['user_id' => $uid, 'order_id' => $order])->first()) {
            ...
            if (ShopConfig::findByCode('use_storage') == '1' && ShopConfig::findByCode('stock_dec_time') == Cart::SDT_PLACE) {
                self::change_order_goods_storage($order, false, Cart::SDT_PLACE);
            }
    ...
    {
        return [
            'id' => $this->attributes['shipping_id'],
    ...
    public function getInvoiceAttribute()
    {
        ...
        if (env('REFRESH_ECSHOP36_DATABASE')) {
            $invoice['number'] = $this->attributes['inv_number'];
        }
    ...
    public function getPromosAttribute()
    {
        ...
        if ($this->attributes['extension_code'] == 'exchange_goods') {
            return [
                ['promo' => 'score', 'price' => $this->attributes['integral']]
            ];
        }
    ...
    public static function order_fee($order, $goods, $consignee, $cart_good_id = 0, $shipping, $consignee_id)
    {
        ...
        foreach ($goods as $key => $val) {
            ...
            $goods_array[$key]['num'] = $val['goods_number'];
            $goods_array[$key]['goods_id'] = $val['goods_id'];
        }
